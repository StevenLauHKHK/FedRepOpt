...
def extract_RepOptGhostNet_scales_from_pth(repoptghostnet, scales_path):
    trained_hs_model = repoptghostnet(mode='hs', num_classes=100)
...
def build_RepOptGhostNet_and_SGD_optimizer_from_pth(repoptghostnet, scales_path, lr, momentum=0.9, weight_decay=1e-5, num_classes=1000):
    model = repoptghostnet(mode='target', num_classes=num_classes)
    scales = extract_RepOptGhostNet_scales_from_pth(repoptghostnet, scales_path=scales_path)
