...
class GhostBottleneck(nn.Module):
    ...
    def __init__(
        ...
        if self.stride > 1:
            ...
            self.bn_dw = nn.BatchNorm2d(int(mid_chs * 2))

        # Squeeze-and-excitation
        if has_se:
            self.se = SqueezeExcite(int(mid_chs * 2), se_ratio=se_ratio)
        else:
            self.se = None
...
def ghostnet(enable_se=True, **kwargs):
    """
    Constructs a RepGhostNet model
    """
    cfgs = [
...
def ghostnet_0_5x(**kwargs):
    return ghostnet(width=0.5, **kwargs)
