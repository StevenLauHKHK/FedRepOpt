...
def build_loader(config, annotations_client_file=None):
    config.defrost()
    dataset_train, config.MODEL.NUM_CLASSES = build_dataset(is_train=True, config=config, annotations_client_file=annotations_client_file)
    config.freeze()
...
def build_loader_central(config):
    config.defrost()
...
def build_dataset(is_train, config, annotations_client_file=None):
    if config.DATA.DATASET == 'imagenet':
        ...
        if config.DATA.ZIP_MODE:
            ...
            if os.path.exists(nori_root):
                ...
                if is_train:
                    dataset = torchvision.datasets.ImageFolder(root=os.path.join(config.DATA.DATA_PATH,'train'), transform=transform)
                else:
                    dataset = torchvision.datasets.ImageFolder(root=os.path.join(config.DATA.DATA_PATH,'val'), transform=transform)
        nb_classes = 1000
        ...
        if config.FL.TRAIN:
            if prefix == 'train' and annotations_client_file :
                dataset = TinyImageNetDataset(
                    os.path.join(config.DATA.DATA_PATH, prefix), 
                    os.path.join(config.DATA.DATA_PATH, config.DATA.ANNOTATIONS_FED, annotations_client_file),
                    transform = transform)
            else:
                dataset = TinyImageNetDataset(os.path.join(config.DATA.DATA_PATH, prefix), os.path.join(config.DATA.DATA_PATH, 'tiny_imagenet-200_' + prefix + '.json'), transform = transform)
            nb_classes = 200
